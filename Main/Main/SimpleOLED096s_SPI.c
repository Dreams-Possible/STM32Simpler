...
void OLED0961_SPI_CharacterString(uint8_t Axle_A,uint8_t Axle_B,char*CharacterString,...)
{
	--Axle_A;
	--Axle_B;
	...
	{
    	//显示焦点定位
		OLED0961_SPI_Focus(Axle_A+a,Axle_B);
		//分别发送字符串的上下两部分
		for(uint8_t b=0;b<strlen((char*)StringData);++b)
		{
			...
			{
				OLED0961_SPI_Data(OLED096_Character_ASCII_1608[StringData[b]-' '][c+a*8]);
			}
		}
	}
}
