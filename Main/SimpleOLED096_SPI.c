#include"SimpleOLED096_SPI.h"
#ifdef SimpleOLED096_SPI_H
...
void OLED096_CharacterString_SPI(uint8_t Axle_A,uint8_t Axle_B,char*CharacterString,...)
{
	--Axle_A;
	--Axle_B;
	...
	{
    	//显示焦点定位
		OLED096_Focus_SPI(Axle_A+a,Axle_B);
		//分别发送字符串的上下两部分
		for(uint8_t b=0;b<strlen((char*)StringData);++b)
		{
			...
			{
				OLED096_Data_SPI(OLED096_Character_ASCII_1608[StringData[b]-' '][c+a*8]);
			}
		}
	}
}
...
void OLED096_Printf_SPI(const char *CharacterString,...)
{
